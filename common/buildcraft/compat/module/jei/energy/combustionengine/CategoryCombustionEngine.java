...
//        return new TextComponent("Combustion Engine Fuels");
        return Component.translatable("buildcraft.jei.title.combustion_engine_fuels");
    }
    ...
    @Override
    public void draw(IFuel fuel, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        PoseStack poseStack = guiGraphics.pose();
//        this.flame.draw(stack, 2, 0);
        this.burnTimeDrabableMap.get(fuel.getTotalBurningTime()).draw(guiGraphics, 2, 0);
...
//        this.font.draw(stack, "Burns for " + fuel.getTotalBurningTime() / 20 + "s", 0, 0, Color.darkGray.getRGB());
        guiGraphics.drawString(this.font, Component.translatable("buildcraft.jei.title.combustion_engine_fuels.burn_time", fuel.getTotalBurningTime() / 20).getString(), 0, 0, Color.darkGray.getRGB(), false);
//        this.font.draw(stack, " at " + MjAPI.formatMj(fuel.getPowerPerCycle()) + " MJ/t", 0, font.lineHeight, Color.darkGray.getRGB());
        guiGraphics.drawString(this.font, Component.translatable("buildcraft.jei.title.combustion_engine_fuels.burn_speed", MjAPI.formatMj(fuel.getPowerPerCycle())).getString(), 0, font.lineHeight, Color.darkGray.getRGB(), false);
...
//        this.font.draw(stack, " total " + MjAPI.formatMj(fuel.getPowerPerCycle() * (long) fuel.getTotalBurningTime()) + " MJ", 1, 2, Color.gray.getRGB());
        guiGraphics.drawString(this.font, Component.translatable("buildcraft.jei.title.combustion_engine_fuels.burn_total", MjAPI.formatMj(fuel.getPowerPerCycle() * (long) fuel.getTotalBurningTime())).getString(), 1, 2, Color.gray.getRGB(), false);
