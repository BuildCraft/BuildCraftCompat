...
//        return new TextComponent("Integration Table");
    ...
    public void setIngredients(IntegrationRecipe recipe, IIngredients ingredients) {
        ingredients.setInputIngredients(Stream.concat(Lists.newArrayList(recipe.getCenterStack().ingredient).stream(), recipe.getRequirements().stream().map(is -> is.ingredient)).collect(Collectors.toList()));
        ingredients.setOutput(VanillaTypes.ITEM, recipe.getExampleOutput());
    }
...
//        List<List<ItemStack>> inputs = ingredients.getInputs(ItemStack.class);
        ...
        List<ItemStack> center = inputs.get(0);
        List<List<ItemStack>> surroundings = inputs.subList(1, inputs.size());
...
//                }
                if (x == 1 && y == 1) {
                    guiItemStacks.init(inventoryIndex, true, 11 + x * 25, 3 + y * 25);
                    guiItemStacks.set(inventoryIndex, center);
                } else if (surroundingsIndexCounter < surroundings.size()) {
                    guiItemStacks.init(inventoryIndex, true, 11 + x * 25, 3 + y * 25);
                    guiItemStacks.set(inventoryIndex, surroundings.get(surroundingsIndexCounter));
        ...
        guiItemStacks.init(inventoryIndex, false, 130, 28);
